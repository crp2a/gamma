...
Once the 'gamma' package is loaded (`library(gamma)`), processing a spectrum is achieved in three steps (detailed hereafter): import, energy calibration and dose-rate estimation.

```{r workflow, echo=FALSE, fig.cap="A typical workflow within the 'gamma' package."}
knitr::include_graphics("./man/figures/PAPER-workflow.jpg")
```

All theses steps can be performed in a GUI with: 
...
The 'gamma' package only supports the most common type of spectrum files: Canberra CAM (.CNF) and Programmers Toolkit files (.TKA). This prevents spending time on data transformation and preserves file metadata. The `read()` function allows to import a CNF-file with the path of the file as a single argument. If the path point to a directory instead of a single file, all spectra will be imported.
...
Spectra are stored as instances of an S4 class. Many mutator methods and group generic functions are available for basic operations and data wrangling. The `plot(spc)` method allows to graphically inspect the data.

### Step 2: adjust the energy scale
The energy calibration of a spectrum is a challenging part but must be performed before dose rate estimation as discussed earlier. The solution presented here requires the user to specify the position (channel) of at least three observed peaks and their corresponding theoretical energies (in keV). A second order polynomial model is fitted on these energy *vs* channel values, then used to predict the new energy scale of the spectrum.

The package allows the user to provide the channel-energy pairs to be used. However, the spectrum can be noisy so it is difficult to properly determine the peak channel. In this case, a better approach may be to pre-process the spectrum (variance-stabilization, smoothing and baseline correction) and perform a peak detection (fig. \@ref(fig:plot)). Once the identified peaks are satisfactory, the user can set the corresponding energy values (in keV) and use these lines to calibrate the energy scale of the spectrum. All signal processing methods are prefixed with `signal_`.
...
Smoothing
: Counting artefacts can be removed by smoothing intensities with `signal_smooth()`. Several methods are implemented here: (weighted) sliding-average and Savitzky-Golay filter [@gorry1990; @savitzky1964]. The example below uses the Savitzky-Golay filter as this method usually provides better results, at the expense of a longer computing time.
...
Energy calibration
: The energy scale calibration needs the energy values of detected peaks to be set. Then `calibrate_energy()` fits a second order polynomial model to predict the new spectrum energy scale.
...
```{r plot, echo=FALSE, fig.width=7.5, fig.height=4, fig.cap="(A) Raw gamma ray spectrum from in situ measurement; (B) Processed spectrum used for peak finding along with the detected peaks."}
cowplot::plot_grid(
...
```{r curve, echo=FALSE, fig.width=3.5, fig.height=3.5, fig.cap="Built-in LaBr calibration curve in used at the CRP2A laboratory (Bordeaux)."}
plot(BDX_LaBr_1) + 
  ggplot2::theme_bw()
```
...
The 'gamma' package allows batch processing of spectra. However, it is strongly recommended to check the result before proceeding. Manual verification is strongly recommended since automatic detection can sometimes produce unreliable results, especially when the processed spectra display poor signal-to-noise ratios. In the future, improvement of the automatic peak detection process is to be expected.
