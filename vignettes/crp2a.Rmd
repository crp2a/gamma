...
calib_dir <- system.file("extdata/crp2a/references", package = "gamma")
...
for (i in 1:length(calib_peaks)) {
  ...
  plot_zoom <- cowplot::plot_grid(
    plot(calib_peaks[[i]]) + plot_info + plot_nolegend +
      ggplot2::coord_cartesian(xlim = c(220, 260), 
                               ylim = c(0, coef(calib_peaks[[i]]@model[[1]])[3])),
    plot(calib_peaks[[i]]) + plot_info + plot_nolegend +
      ggplot2::coord_cartesian(xlim = c(1400, 1550), 
                               ylim = c(0, coef(calib_peaks[[i]]@model[[2]])[3])),
    plot(calib_peaks[[i]]) + plot_info + plot_nolegend +
      ggplot2::coord_cartesian(xlim = c(2500, 2700), 
                               ylim = c(0, coef(calib_peaks[[i]]@model[[3]])[3])),
    ncol = 3
  )
  # Multiple plots
  plot_spc <- cowplot::plot_grid(
    plot(calib_peaks[[i]]) + plot_info +
  ...
  print(
    knitr::kable(
      ...
      caption = paste(calib_peaks[[i]][["spectrum"]][["reference"]],
                      "Estimated peak positions", sep = " - ")
    )
  )
  cat('\n')
}
