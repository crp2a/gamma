...
### Linear regression
```{r calibration, fig.width=5, fig.height=5}
# Build calibration curve
(BDX1 <- fit(
  calib_scaled,
  noise_scaled,
...
```{r lm-check, echo=FALSE, fig.width=7, fig.height=7}
...
criterion_aic <- c(stats::AIC(BDX1[["model"]]), stats::AIC(fit))
criterion_bic <- c(stats::BIC(BDX1[["model"]]), stats::BIC(fit))
