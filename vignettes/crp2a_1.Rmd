...
calib_spc[["BRIQUE"]] %>%
  ...
  integrate_signal(range = c(165, 2800), noise = c(25279.63171, 1.66235))
```
...
# Display curve data
knitr::kable(BDX100_data, row.names = FALSE)
```

```{r lm-summary, echo=FALSE}
# Slope: 31.16884 / Intercept: 2006.62935
summary(BDX100_curve[["model"]])
```

### Check model
```{r lm-check, echo=FALSE, out.width='47%', fig.width=3.5, fig.height=3.5, fig.show='hold'}
calib_residuals <- data.frame(
  name = BDX100_curve[["data"]]$name,
  fitted = BDX100_curve[["model"]]$fitted.values,
  residuals = BDX100_curve[["model"]]$residuals,
  standardized = stats::rstandard(BDX100_curve[["model"]]),
  cook = stats::cooks.distance(BDX100_curve[["model"]]),
...
criterion_aic <- c(stats::AIC(BDX100_curve[["model"]]))
criterion_bic <- c(stats::BIC(BDX100_curve[["model"]]))

criteria <- data.frame(
  AIC = criterion_aic,
  D_AIC = criterion_aic - min(criterion_aic),
  BIC = criterion_bic,
  row.names = c("New curve (BDX100_curve)", "Old curve (VBA macro)")
)
