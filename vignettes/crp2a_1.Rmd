...
summary(BDX100[["model"]])
```

### Check model
```{r lm-check, echo=FALSE, out.width='47%', fig.width=3.5, fig.height=3.5, fig.show='hold'}
calib_residuals <- data.frame(
  name = BDX100[["data"]]$name,
  fitted = BDX100[["model"]]$fitted.values,
  residuals = BDX100[["model"]]$residuals,
  standardized = stats::rstandard(BDX100[["model"]]),
  cook = stats::cooks.distance(BDX100[["model"]]),
...
criterion_aic <- c(stats::AIC(BDX100[["model"]]))
criterion_bic <- c(stats::BIC(BDX100[["model"]]))

criteria <- data.frame(
  AIC = criterion_aic,
  D_AIC = criterion_aic - min(criterion_aic),
  BIC = criterion_bic,
  row.names = c("New curve (BDX100)", "Old curve (VBA macro)")
)
