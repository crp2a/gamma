...
plot(BDX_LaBr_1, threshold = "Ni") +
  ggplot2::theme_bw()
plot(BDX_LaBr_1, threshold = "NiEi") +
  ggplot2::theme_bw()
```
...
Ni_model <- get_model(BDX_LaBr_1, "Ni")
Ni_residuals <- data.frame(
  name = BDX_LaBr_1[["data"]]$name,
...
NiEi_model <- get_model(BDX_LaBr_1, "Ni")
...
criterion_aic <- c(stats::AIC(BDX_LaBr_1[["model"]]))
criterion_bic <- c(stats::BIC(BDX_LaBr_1[["model"]]))

criteria <- data.frame(
  AIC = criterion_aic,
  D_AIC = criterion_aic - min(criterion_aic),
  BIC = criterion_bic,
  row.names = c("New curve (BDX_LaBr_1)", "Old curve (VBA macro)")
)
