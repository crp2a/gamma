...
# Import a CNF file
cnf_test <- system.file("extdata/test_LaBr.CNF", package = "gamma")
(spc_cnf <- read(cnf_test))

# Import a TKA file
tka_test <- system.file("extdata/test_LaBr.TKA", package = "gamma")
...
sliced <- slice_signal(spc_tka)
```

### Stabilization
```{r stabilize}
# Use a square root transformation
transformed <- stabilize_signal(sliced, transformation = sqrt)
```

### Smoothing
```{r smoothing}
# Use a 21 point Savitzky-Golay-Filter to smooth the spectrum
smoothed <- smooth_signal(transformed, method = "savitzky", m = 21)
```
...
scaled2 <- calibrate_energy(spc_tka, lines = calib_lines)
```
...
data(BDX_LaBr_1_curve) # IRAMAT-CRP2A (Bordeaux)
data(AIX_NaI_curve) # CEREGE (Aix-en-Provence)
```
...
data("clermont")
set_dose(calib_spc) <- clermont[, c("gamma_dose", "gamma_error")]
...
calib_curve <- fit_dose(
  ...
  Ni_range = c(300, 2800), # Set the integration range (in keV)
  NiEi_noise = c(25280, 1.66),
...
# Estimate the gamma dose rate
dose_rate <- predict_dose(calib_curve, test_spc, simplify = TRUE)
```
