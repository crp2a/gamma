...
# usethis::use_data(AIX100_curve, overwrite = FALSE)
```

### Check model
```{r lm-summary, echo=FALSE}
summary(AIX100_curve[["model"]])
```

```{r lm-check, echo=FALSE, out.width='47%', fig.width=3.5, fig.height=3.5, fig.show='hold'}
calib_residuals <- data.frame(
  name = AIX100_curve[["data"]]$name,
  fitted = AIX100_curve[["model"]]$fitted.values,
  residuals = AIX100_curve[["model"]]$residuals,
  standardized = stats::rstandard(AIX100_curve[["model"]]),
  cook = stats::cooks.distance(AIX100_curve[["model"]])
)
...
criteria <- data.frame(
  AIC = c(stats::AIC(AIX100_curve[["model"]])),
  BIC = c(stats::BIC(AIX100_curve[["model"]])),
  row.names = c("AIX1")
)
