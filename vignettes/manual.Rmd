...
spc_cnf <- read(cnf_test, skip = 1:35)
spc_cnf

# Plot spectra
plot(spc_cnf, xaxis = "chanel", yaxis = "rate") +
  ggplot2::labs(x = "Chanel", y = "Count rate [1/s]") +
  ggplot2::theme_bw()
```
...
peak_fit_auto <- fitPeaks(peak_index, scale = "chanel")
...
```{r peak-fit-manual, fig.width=7, fig.height=5, fig.align="center", fig.show="hold"}
# Fit peaks at a given energy (keV)
peak_fit_energy <- fitPeaks(spc_clean1, 
                            peaks = c(238, 1461, 2614.5), 
                            scale = "energy")

plot(peak_fit_energy) +
  ggplot2::labs(title = "Fixed peak positions (keV)", x = "Energy [keV]", y = "Count") +
  ggplot2::theme_bw()

# Fit peaks at a given chanel
peak_fit_chanel <- fitPeaks(spc_clean1,
                            peaks = c(86, 496, 876),
                            scale = "chanel")
...
## Get peak positions
knitr::kable(
  peak_fit_chanel[["peaks"]],
  digits = 3,
  col.names = c("Chanel", "Energy [keV]", "Count", "Count rate [1/s]"),
  caption = "Peak positions (NLS estimates with starting chanels)."
)
```
...
## Signal integration
```{r integrate}
tka_int1 <- integrateSignal(tka_scale, range = c(200, 2800), noise = NULL)
tka_int1

tka_int2 <- integrateSignal(tka_scale, range = c(200, 2800), noise = c(1114.769, 0.5378))
tka_int2
```
...
spc_cerege <- read(cnf_cerege, skip = 1:35)
spc_cerege
...
# Visual inspection of energy calibration
plot(noise_scale) +
  ggplot2::geom_vline(xintercept = c(238, 1461, 2614.5), linetype = 3) +
  ggplot2::labs(x = "Energy [keV]", y = "Count") +
  ggplot2::theme_bw()
