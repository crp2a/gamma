...
# Detect peaks
peak_index <- findPeaks(spc_clean1, span = 20)

plot(spc_clean1, select = 1:3, facet = TRUE) +
  ggplot2::labs(x = "Energy [keV]", y = "Counts") +
  ggplot2::geom_vline(xintercept = peak_index$energy, colour = "red") +
  # ggplot2::geom_hline(yintercept = MAD(spc_clean1@counts)*2, colour = "blue") +
  ggplot2::theme_bw()
```
...
noise <- integrateSignal(spc_PB, peaks = c(238, 1461, 2614.5), span = 50)
noise
```
