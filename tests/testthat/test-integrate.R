...
test_that("Integrate GammaSpectra", {
  ...
  int_mtx <- integrateSignal(spectra, range = c(200, 2800), simplify = TRUE)
  expect_is(int_mtx, "matrix")
  expect_equal(ncol(int_mtx), 2)
  expect_equal(nrow(int_mtx), length(spectra))

  int_list <- integrateSignal(spectra, range = c(200, 2800), simplify = FALSE)
