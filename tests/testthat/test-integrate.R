...
test_that("Integrate GammaSpectrum", {
  ...
  int1 <- integrate_signal(cnf, range = c(200, 2800), energy = TRUE)
  expect_equivalent(int1, c(1.483392e+05, 9.361146e+00),
                    tolerance = 1e-06)
  expect_length(int1, 2)

  int2 <- integrate_signal(cnf, bkg, range = c(200, 2800), energy = FALSE)
...
test_that("Integrate GammaSpectra", {
  ...
  int1 <- integrate_signal(cnf, range = c(200, 2800), simplify = TRUE)
  expect_type(int1, "double")
  expect_equal(ncol(int1), 2)
  expect_equal(nrow(int1), length(cnf))

  int2 <- integrate_signal(cnf, bkg, range = c(200, 2800), simplify = FALSE)
